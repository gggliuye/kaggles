...
tf.app.flags.DEFINE_string('data_dir', 'iceberg_data/',
...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights', shape=[5, 5, 2, 32],
                                         stddev=1e-4, wd=0.0)
  ...
  with tf.variable_scope('local3') as scope:
    ...
    reshape = tf.reshape(pool3, [-1,6400])

    weights = _variable_with_weight_decay('weights', shape=[6400, 192],
  ...
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [10, 2],
